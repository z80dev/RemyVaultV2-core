...
contract MintOutSimulations is BaseTest, DerivativeTestUtils {
    ...
    function _runScenario(ScenarioConfig memory config) internal {
        ...
        (bool derivativeIsCurrency0,) = _predictDerivativeOrientation(config.maxSupply, parentState.parentVault);
        ...
        params.salt = mineSaltForToken1(factory, parentState.parentVault, params.maxSupply);
        ...
        ScenarioResult memory result = _simulateMintOut(
            ...
            derivativeVault,
            parentIsCurrency0,
            initialParentSqrtPrice,
            initialDerivativeSqrtPrice
        );

        _reportScenario(
            config, result, parentIsCurrency0, derivativeIsCurrency0, parentState.parentVault, derivativeVault
        );
    }
    ...
    function _reportScenario(
        ...
        console.log(
            "Total derivatives acquired:",
            result.derivativesBought / 1e18,
            ".",
            (result.derivativesBought % 1e18) / 1e16
        );
        ...
        uint256 percentMinted =
            result.derivativesBought == 0 ? 0 : FullMath.mulDiv(result.derivativesBought, 10000, derivativeSupply);
        ...
        _logPriceImpact(
            "Derivative/Parent",
            result.initialDerivativeSqrtPrice,
            result.finalDerivativeSqrtPrice,
    ...
    function _setupParentPool() internal returns (ParentPoolState memory state) {
        ...
        uint128 liquidity = LiquidityAmounts.getLiquidityForAmounts(
            parentSqrtPrice, sqrtLower, sqrtUpper, PARENT_ETH_LIQUIDITY, PARENT_TOKEN_LIQUIDITY
        );
    ...
    function _predictDerivativeOrientation(uint256 maxSupply, address parentVault)
        internal
        view
        returns (bool derivativeIsCurrency0, address predictedVault)
    {
        // Mine salt to ensure derivative is always token1 (currency1)
        bytes32 salt = mineSaltForToken1(factory, parentVault, maxSupply);
        (, address predictedVaultAddr) = predictDerivativeAddresses(factory, maxSupply, salt);
