...
contract InventoryMetavaultTest is Test {
    ...
    function setUp() public {
        ...
        coreVault = IRemyVault(address(new RemyVaultSol("REMY", "REMY", address(nft))));
        UNIT = coreVault.quoteDeposit(1);
    ...
    function testYieldAccumulation() public {
        ...
        _fundRemy(charlie, purchasePrice);
    ...
    function testAllFailureCases() public {
        ...
        _fundRemy(bob, 10000 * UNIT);

        vm.startPrank(bob);
        coreVault.approve(address(metavault), 10000 * UNIT);
    ...
    function _fundRemy(address user, uint256 amount) internal {
        deal(address(coreVault), user, amount, true);
    }
