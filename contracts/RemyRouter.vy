...
@external
def set_addresses(vault_addr: address, weth_addr: address, v3router_addr: address, erc4626_addr: address):
    """
    Initializes the contract with the addresses of the vault, router, and tokens
    """
    assert msg.sender == self.owner, "!owner"
    self.vault_address = vault_addr
    self.weth = weth_addr
    self.v3router_address = v3router_addr
    self.erc4626_address = erc4626_addr
    self.erc721_address = staticcall RemyVault(vault_addr).erc721()
    self.erc20_address = staticcall RemyVault(vault_addr).erc20()
...
def stake_inventory(recipient: address, token_ids: DynArray[uint256, 100]):
    ...
    extcall nft.setApprovalForAll(self.vault_address, True)
